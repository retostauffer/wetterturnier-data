...
def readconfig( file = 'config.conf' ):
   ...
   # - Which stations should be migratet into the archive table
   try:
      config['cleanup_stations'] = []
      tmp = CNF.get('cleanup','stations')
      for rec in tmp.split(','):
         if len(rec.strip()) == 0: continue
         config['cleanup_stations'].append( int(rec.strip()) )
   except:
      sys.exit("[!] ERROR: misconfiguration in config file, cleanup stations list")
